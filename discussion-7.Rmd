...
We want to estimate the $\phi$ parameters in the acceleration factor. It turns out that for the Weibull parameterization (includes exponential), $\phi = \beta$. So HR = acceleration factor.

```{r}
mod <- flexsurvreg(Surv(time, status) ~ trt, 
                   data=diabetic, dist="exp")
mod$res
```

## Estimating acceleration factors on time scale

We fit a proportional hazards model and estimated the hazard ratio, but because of the relationship between the PH and AFT models for the Weibull family distributions, but we could just take $A = \exp(-0.81169)$ to get the acceleration factor. So time is accelerated by a factor of 0.444 in the treated group (so, decelerated).

Translates to average survival time in the treated group being 1/0.444 = 2.25 times as long as that in the untreated group.
